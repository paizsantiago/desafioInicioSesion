...
function connectMG(){
  try {
      connect("mongodb+srv://santiagopaiz:7pUEtOwIzTYvQyOF@cluster0.cpghy3l.mongodb.net/?retryWrites=true&w=majority", {useNewUrlParser: true});
  } catch (e) {
      console.log(e);
...
app.post('/login', passport.authenticate("login", { failureRedirect: "/loginErrorAuth" }), (req, res) => {
  const {username, password} = req.user;
  const user = {username, password};
...
app.post('/register', passport.authenticate("signup", { failureRedirect: "/registerErrorAuth" }), (req, res) =>{
...
//   console.log(first)
//   res.render('logout.pug' , {user: user})
// })
